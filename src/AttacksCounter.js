class AttackCounter {
  ...
  exec() {
    ...
    simplePopup.onsubmit = e => {
      ...
      this.#minPopulation      = minPopulationValue !== "" ? Number(minPopulationValue) : 0,
      this.#maxPopulation      = maxPopulationValue !== "" ? Number(maxPopulationValue) : Infinity;
      this.#fetchCommandsPerPlayer(inputCoords);
    }
  }

  #fetchCommandsPerPlayer(inputData) {
    ...
    fetch(url)
      ...
      .then(async body => {
        ...
        for (const coord in villageData) {
          ...
          const result = await this.#fetchVillageData(id, coord);
        ...
        this.#downloadFile(summaryFilename, summaryCsv);
        ...
        this.#downloadFile(villageFilename, villageCsv);
      })
      .catch(error => {
  ...
  async #fetchVillageData(villageId, coordinates) {
    const url = `https://${game_data.world}.plemiona.pl/game.php`
    ...
    try {
      ...
      for (const element of commandLinks) {
        ...
        const commmandStrength = this.#calculateCommandStrength(commandTables[1]);
        if (commmandStrength < this.#minPopulation || commmandStrength > this.#maxPopulation)
          continue;
  ...
  #calculateCommandStrength(commandTable) {
