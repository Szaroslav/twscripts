if (!$("#planer_klinow").length) {
  ...
  for (var i = 0; i < dane.predkosci.length; i++) {
...
function wypiszMozliwosci() {
  ...
  for (i = 0; i < mojeWioski.length; i++) {
    ...
    var najwolniejsza = 0;
    var mozliwewojska = "&from=simulator";
    ...
    for (j = 0; j < dane.predkosci.length; j++) {
      ...
      var a = Math.abs(Number(cel[0]) - mojeWioski[i][mojeWioski[i].length - 3]);
      var b = Math.abs(Number(cel[1]) - mojeWioski[i][mojeWioski[i].length - 2]);
      var czasPrzejscia = Math.sqrt((a * a) + (b * b)) * dane.predkosci[j] * 60;
    ...
    if (najwolniejsza != 0) {
      ...
      var ddd = tmp.getDate() + "." + (tmp.getMonth() + 1) + "\u00A0" + tmp.getHours() + ":" + tmp.getMinutes() + ":" + tmp.getSeconds();
...
//   var el = document.getElementById("wyborWojsk");
...
//     } else
//       for (var j = 0; j < minimalna_ilosc_wojsk.length; j++)
...
function wybieranieWiosek() {
  ...
  var okienko = "<tr><th style=\"cursor:pointer;\" onclick=\"ustaw_min(0); $('#wyborWojsk tr:has(td)').each(function(i){$(this).show();}); \">Minimalna\u00A0ilość\u00A0wojsk:";
  for (i = 0; i < dane.predkosci.length; i++)
  ...
  for (i = 0; i < wojska.length; i++) {
    var ukryty = false;
    var komorki = "<a href=" + dane.linkDoPrzegladuWioski + id[i] + ">" + nazwyWiosek[i].replace(/\s+/g, "\u00A0") + "</a>";
...
function rysujPlaner() {
  ...
  if ($(".no_ignored_command").length)
    $(".no_ignored_command").each(function() {
      if ($(this).html().match("snob.png") && !pobralemCzas) {
        var czas_wejscia_grubego = $(this).find("td:eq(2)").text().match(/\d+/g);
  ...
  function processResponse() {
    if (r.readyState == 4 && r.status == 200) {
      ...
      for (i = 1; i < tabela.rows.length; i++) {
        ...
        wojska[i - 1] = [];
        var pustaWioska = 0;
        for (var j = 2; j < tabela.rows[i].cells.length - 1; j++) {
